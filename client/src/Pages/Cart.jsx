          ...
          <ul className=" flex flex-col flex-2">
            {cart.map((item) => (
              <Link to={`/productDetails/${item.id}`}>
                <div
                  key={item.id}
                  className="flex justify-between items-start bg-white p-4 rounded  border-1 border-gray-200 "
                >
                  <div className="flex flex-row ">
                    {/*IMAGE*/}
                    <div className="flex-shrink-0 mr-4 md:h-32w-32 h-15 w-15 bg-gray-200 rounded mb-4">
                      {item.image_url ? (
                        <img
